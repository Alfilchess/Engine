     ...
     // if(File.Exists(tablePath))
      {
        ...
        switch(pieces)
        {
          case "kpk": currentStream=new MemoryStream(Motor.source.Resource.Resource.kpk_gtb); break;
          case "kbk": currentStream=new MemoryStream(Motor.source.Resource.Resource.kbk_gtb); break;
          case "kbkb":
            currentStream=new MemoryStream(Motor.source.Resource.Resource.kbkb_gtb);
            break;
          case "kbkn":
            currentStream=new MemoryStream(Motor.source.Resource.Resource.kbkn_gtb);
            break;
          case "kbnk":
            currentStream=new MemoryStream(Motor.source.Resource.Resource.kbnk_gtb);
            break;
         
          case "knkn":
            currentStream=new MemoryStream(Motor.source.Resource.Resource.knkn_gtb);
            break;
         
          case "kqk":
            currentStream=new MemoryStream(Motor.source.Resource.Resource.kqk_gtb);
            break;
          case "kqkr":
            currentStream=new MemoryStream(Motor.source.Resource.Resource.kqkr_gtb);
            break;
          case "krk":
            currentStream=new MemoryStream(Motor.source.Resource.Resource.krk_gtb);
            break;
          case "krkb":
            currentStream=new MemoryStream(Motor.source.Resource.Resource.krkb_gtb);
            break;
          case "kpkp":
            currentStream=new MemoryStream(Motor.source.Resource.Resource.kpkp_gtb);
            break;
        }
