...
namespace Motor
{
  ...
  public class cMaterial
  {
    ...
    public class cStructMaterial
    {
      ...
      public cFinalesBase[] m_FactorEscalaFinal = new cFinalesBase[2];
      public int m_nPesos;
      ...
      public val EvalFinal(cPosicion pos)
      {
        return m_FuncionEvalucion.m_Ejecucion(pos);
      }

      public factor FactorDeEscalaFinal(cPosicion pos, color c)
      {
        return m_FactorEscalaFinal[c] == null || m_FactorEscalaFinal[c].m_Ejecucion(pos) == cFactorEscala.NAN
        ? (factor[c]) : m_FactorEscalaFinal[c].m_Ejecucion(pos);
      }
    }
    ...
    public static cFinales[] m_EvalKXK = new cFinales[2] { new cFinales(cColor.BLANCO, stFinalesTipo.KXK), new cFinales(cColor.NEGRO, stFinalesTipo.KXK) };
    public static cFinales[] m_EvalKBPsK = new cFinales[2] { new cFinales(cColor.BLANCO, stFinalesTipo.KBPsK), new cFinales(cColor.NEGRO, stFinalesTipo.KBPsK) };
    public static cFinales[] m_EvalKQKRPs = new cFinales[2] { new cFinales(cColor.BLANCO, stFinalesTipo.KQKRPs), new cFinales(cColor.NEGRO, stFinalesTipo.KQKRPs) };
    public static cFinales[] m_EvalKPsK = new cFinales[2] { new cFinales(cColor.BLANCO, stFinalesTipo.KPsK), new cFinales(cColor.NEGRO, stFinalesTipo.KPsK) };
    public static cFinales[] m_EvalKPKP = new cFinales[2] { new cFinales(cColor.BLANCO, stFinalesTipo.KPKP), new cFinales(cColor.NEGRO, stFinalesTipo.KPKP) };
    ...
    //----------------------------------------------------------------------------------
    public static cMaterial.cStructMaterial BuscarFinal(cPosicion pos, cMaterial.cTablaHashMaterial entradaMaterial, cListaDeFinales finales)
    {
      ...
      e.m_FactorEscalaFinal[0] = e.m_FactorEscalaFinal[1] = null;
      e.m_nPesos = 0;
      e.m_Clave = clave;
      ...
      {
        e.m_FactorEscalaFinal[sf.Ganador()] = sf;
        return e;
      }

      if (IsKBPsKs(pos, cColor.BLANCO))
        e.m_FactorEscalaFinal[cColor.BLANCO] = m_EvalKBPsK[cColor.BLANCO];

      if (IsKBPsKs(pos, cColor.NEGRO))
        e.m_FactorEscalaFinal[cColor.NEGRO] = m_EvalKBPsK[cColor.NEGRO];

      if (IsKQKRPs(pos, cColor.BLANCO))
        e.m_FactorEscalaFinal[cColor.BLANCO] = m_EvalKQKRPs[cColor.BLANCO];

      else if (IsKQKRPs(pos, cColor.NEGRO))
        e.m_FactorEscalaFinal[cColor.NEGRO] = m_EvalKQKRPs[cColor.NEGRO];
      ...
      if (npm_w + npm_b == cValoresJuego.CERO && pos.GetNumPiezas(cPieza.PEON) != 0)
      {
        if (0 == pos.GetNum(cColor.NEGRO, cPieza.PEON))
        {
          ...
          e.m_FactorEscalaFinal[cColor.BLANCO] = m_EvalKPsK[cColor.BLANCO];
        }
        else if (0 == pos.GetNum(cColor.BLANCO, cPieza.PEON))
        {

          e.m_FactorEscalaFinal[cColor.NEGRO] = m_EvalKPsK[cColor.NEGRO];
        }
        else if (pos.GetNum(cColor.BLANCO, cPieza.PEON) == 1 && pos.GetNum(cColor.NEGRO, cPieza.PEON) == 1)
        {

          e.m_FactorEscalaFinal[cColor.BLANCO] = m_EvalKPKP[cColor.BLANCO];
          e.m_FactorEscalaFinal[cColor.NEGRO] = m_EvalKPKP[cColor.NEGRO];
        }
      }
